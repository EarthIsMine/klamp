...
The eight-step trace launches a token, atomically issues its capped-hook identity and canonical pool records, shows an aggregator discovering both issuer and replica pools, excludes the unregistered replica before execution, verifies the wrapper bytecode, return-delta permissions, and immutable 100 bps cap, prices the accepted route at that maximum, and only then forwards its PoolKey through the guarded router to PoolManager. It then simulates a compromised strategy admin producing a 3,000 bps request, compares the protected 100 bps result with an unguarded 3,000 bps loss, and revokes the hook identity so guarded routing blocks it immediately. The progress rail advances the next unlocked step and lets presenters revisit any completed step directly; `Previous` moves back one scene without discarding the completed trace. The demo uses the typed mock client: no wallet, RPC, ENS lookup, or transaction is performed, and mock addresses or receipts are not live-chain claims.
