...
- The New pool tab can also open the pool with `QuoteAwareFeeHook` (dynamic fee: 0.05% when V4Quoter asks, 10% on a real swap) once it is deployed with `contract/script/DeployQuoteAwareHook.s.sol`; its address is fixed in advance. Without Klamp that pool wins the quote, reverts at 5% slippage and pays about 10% less than quoted at 15%; with Klamp it is crossed out. Otherwise the look-alike uses the same honest `DeltaFeeHook`, so the naive route really does pay out its quote. The point is that Klamp only trusts the pool the issuer declared; a hook that charges more at swap time than at quote time is not deployed on Sepolia. It is reproduced on v4-core in [`contract/test/QuoteDivergenceAttack.t.sol`](../contract/test/QuoteDivergenceAttack.t.sol), and the Swap tab says so under the route: without Klamp, a note that the undeclared pool's quote can shrink to the slippage minimum; with Klamp, how much output it gave up versus the best quote and why.
