...
import {CanonicalPoolRegistrar, IUERC20Factory, IPoolManager} from "../src/CanonicalPoolRegistrar.sol";
contract DeployPhase1 is Script {
    function run() external returns (Phase1Setup.Deployment memory d) {
        ...
        c.registryImpl = vm.envAddress("USER_REGISTRY_IMPL");
        c.resolverImpl = vm.envAddress("RESOLVER_IMPL");
        c.poolManager = IPoolManager(vm.envAddress("POOL_MANAGER"));
        ...
        require(address(c.factory).code.length > 0 && c.registryImpl.code.length > 0 && c.resolverImpl.code.length > 0, "missing protocol code");
        vm.startBroadcast(c.operator);
        d = Phase1Setup.deploy(c,CanonicalPoolRegistrar(vm.envOr("REGISTRAR",address(0))));
        vm.stopBroadcast();
    }
}
