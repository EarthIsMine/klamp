...
contract LocalPhase1 is Script {
    PoolManager manager;
    StateView state;
    IPermissionedRegistry root;
    IPermissionedRegistry eth;
    IUniversalResolve universal;
    ...
    function run() external {
        ...
        root = EnsDeploy.permissionedRegistry(labels,operator,Phase1Setup.REG_ROLES);
        eth = EnsDeploy.permissionedRegistry(labels,operator,Phase1Setup.REG_ROLES);
        root.register("eth",operator,eth,address(0),Phase1Setup.PARENT_ROLES,type(uint64).max);
        eth.setParent(root,"eth");
        c.factory = new VerifiableFactory(); c.registryImpl = EnsDeploy.userRegistryImpl(labels);
        c.resolverImpl = EnsDeploy.resolverImpl();
        ...
        universal = EnsDeploy.universalResolver(root,operator);
