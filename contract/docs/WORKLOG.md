...
## DEX8 — Familiar layouts, contracts panel and transaction toasts

- Date / environment / tools: 2026-09-27 / Vite 8, Sepolia public RPC, headless Chromium, Lazyweb screen references / Claude Code (Opus 5.5)
- Human decisions/changes: Make each tab read like the product that role would use, without copying any brand: borrow the structure (stacked pay/receive card, create-token form), keep Klamp's colours, name and the explanation on the right; neutral wording for the pool tab; an always-visible testnet marker; explorer links for every contract and transaction in use.
- AI work: Launch is a "Create a coin" form (generated ticker icon, name, ticker, optional description saved to ENS after launch, Create coin); after launch "<TICKER> is live · View on Etherscan". The look-alike tab is renamed "New pool · anyone" and is a "Create a pool" form (ETH / token pair, fee tier, hook with its address, liquidity, starting price); the declare check runs without a wallet from a stand-in address and still reverts `NotIssuer`. Swap has a token button inside the receive box with a token menu (paste address, Etherscan link per token) and max slippage behind a settings button. New `Contracts` panel per tab lists each contract with what it does and how (you sign / inside tx / reads / simulates), linked to Etherscan. Transaction toasts show the destination contract and "View on Etherscan". Header shows "Sepolia testnet · demo". No third-party logo, colour or name is used in the UI.
- AI-run verification: `pnpm build` passed. Headless Chromium screenshots at 1440 px and 390 px of all three tabs, the token menu and the slippage menu with live KHOOK data; "Try to declare it as canonical" without a wallet shows `NotIssuer`. Wallet flows (launch, approve, seed, swap, toasts) were not exercised.
- Human verification: Pending human verification.

## DOC1 — Submission readiness: provenance, specs, demo links

- Date / environment / tools: 2026-09-27 / GitHub CLI / Claude Code (Opus 5.5)
- Human decisions/changes: Checked the repository against the ETHGlobal Tokyo 2026 rules (https://ethglobal.com/events/tokyo2026/info/details: history proving hackathon work, documented pre-existing work, AI use documented with all spec files, prompts and planning artifacts in the repository) and asked for the gaps to be fixed. The team submitted the Uniswap developer feedback form.
...
## ATK2 — The quote/swap fee attack as a live Sepolia pool

- Date / environment / tools: 2026-09-27 / Foundry 1.7.1, solc 0.8.26, Sepolia fork (public RPC), Vite 8 / Claude Code (Opus 5.5)
- Human decisions/changes: The team spec's "Demo 1 attack pool" task: deploy the reproduction (quote 0.05%, swap 10%) on Sepolia and open a look-alike on KHOOK/ETH, so the naive route loses on camera. Deployed from a team member's own account, not the deployment account; no key in the repository.
- AI work: `src/demo/QuoteAwareFeeHook.sol` (beforeSwap only; `sender == V4Quoter` → 0.05%, otherwise 10%, as an LP fee override on a dynamic-fee pool; immutable, no owner). `script/DeployQuoteAwareHook.s.sol` deploys it through the CREATE2 factory at a predicted address, `0x0795D625a3A6D17c073e889e6a2EEE0E1c728080` (salt 0xafc1, low bits 0x80), recorded in `deployments/quote-aware-hook.txt`. `test/QuoteAwareHookFork.t.sol` (fork profile) deploys it on a Sepolia fork, opens ETH/KHOOK through the live PoolSeeder one tick spacing above the declared price with 350,000 KHOOK, then quotes with the live V4Quoter and swaps through the live Universal Router with dex's V4_SWAP encoding. dex's New pool tab gains a "Quote-aware · 0.05% → 10%" hook (enabled once the address has code; dynamic fee, spacing 60, same starting price); `feeLabel` shows "dynamic fee". The CLI's `pools.json` lists the pool as a third candidate. READMEs point to all three files.
- AI-run verification: `FOUNDRY_PROFILE=fork forge test --match-contract QuoteAwareHookFork -vv` 2/2 at Sepolia ~11788150: declared quote 196,041 KHOOK, attack quote 197,363, swap reverts at 5% slippage, receives 178,011 at 15% (9.80% below its quote, below the declared pool's quote); the declared pool pays exactly its quote. A first run starting at the declared price won the quote by only 0.08% (thin liquidity), so the start moved up one spacing. `forge script script/DeployQuoteAwareHook.s.sol --rpc-url sepolia` (no broadcast) simulates to the predicted address. CLI dry run lists the new candidate (quote `fail` until the pool exists). dex `pnpm build` passed. Not yet run: the Sepolia broadcast and opening the pool from dex (need a wallet).
- Human verification: Pending human verification.
- Sepolia deployment (human, team member's own account): `forge script script/DeployQuoteAwareHook.s.sol --rpc-url sepolia --broadcast --interactive`, tx `0x20e2efa6c53bb837172468b59ea5818d98c798f7d1d6e59fc540cc1312e20575`, block 11788316, 470,150 gas, at the predicted address `0x0795D625a3A6D17c073e889e6a2EEE0E1c728080`. The first attempt passed `--interactive 1`; in Foundry 1.7.1 `--interactive` takes no value, so `1` became a script argument ("encode length mismatch") and nothing was sent. AI check after deployment: code present; `quoter()` = V4Quoter, `quoteFee()` = 500, `swapFee()` = 100000, `poolManager()` = PoolManager. The broadcast record (no key fields) is copied to `docs/evidence/sepolia-broadcast/DeployQuoteAwareHook/`. The pool itself is still to be opened from dex.

## DEX11 — Say why a swap would revert, with numbers

- Date / environment / tools: 2026-09-27 / Vite 8, Sepolia public RPC, Node 24 / Claude Code (Opus 5.5)
- Human decisions/changes: After opening the quote-aware pool from dex, a naive swap showed only MetaMask's "An internal error was received".
- Finding (read-only, Sepolia): the attack pool `0x80c1dc21…` (dynamic fee, QuoteAwareFeeHook) quotes 197,396.57 KHOOK for 0.0005 ETH against the declared pool's 195,964.04, so the naive route picks it. An eth_call of dex's Universal Router calldata reverts at 5% slippage with `V4TooLittleReceived(min ≈ 187,527, received ≈ 177,6xx)` and succeeds at 15%. The wallet's gas estimate hits that revert and reports a generic internal error. This is the attack working as designed.
