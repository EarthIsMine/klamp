...
- 프론트의 온체인 데이터 계층은 `contract/sdk`의 상태를 그대로 소비한다. 대표 풀 조회는 1단계 설계 문서(`contract/final_klamp_with_code.md`)의 `registered | not_registered | lookup_failed`, 경로 비교는 `match | mismatch | blocked`를 유지한다.
