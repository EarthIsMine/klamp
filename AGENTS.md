...
- Keep the Phase 1 contract's spec, work records and implementation inside `contract/`, the frontend inside `web/`, and the wallet dApp inside `dex/`.
- Before working in `contract/`, read and apply `contract/AGENTS.md` and `contract/Klamp_Phase1_Agent_Spec.md`.
- Run `contract/` commands from that directory with npm/Foundry, and `web/` and `dex/` commands from their own directories with pnpm. Do not merge one side's package manager or lockfile into the other.
